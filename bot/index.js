...
© ${ownerName}`;
      ...
      // Auto-join group on startup with random cute messages
setTimeout(async () => {
  try {
    const { GROUP_INVITE_CODE } = await import('./lib/channelConfig.js');
    
    // Yahan aapke saare favourite messages ki list hai
    const messages = [
      `lost in your rhythm, i found my way, 🍃
Shahzad's bot is online, just to brighten your day.. 🥃✨
stay as sweet as you are.. 🤍🍭`,
      `i was just a heartbeat away, waiting for the right moment to return to you.. 🍃

Shahzad's bot is officially online, keeping the energy as deep and mysterious as the night. 🥃🐚
Not just connected to the internet, but connected to the soul of this group.. 🤍✨

stay as beautiful as the silence before the dawn. 🕊️`,
      `lost in the rhythm, but always finding my way back to you.. 🍃

Shahzad's bot is online, breathing life into the chat again. 🥃
smooth, steady, and maybe a little bit in love with this vibe.. ✨🐚

stay beautiful, inside and out. 🤍`,
      `just like a heartbeat you can’t ignore.. i’m back. 🥃✨

Shahzad's bot is connected, making sure the energy stays as sweet as you are. 🕊️
let’s make this moment feel like forever.. 🤍`, `i couldn't stay away for too long.. ✨

Shahzad's bot is back, just to be near you all again. 🥃
keeping the vibes soft and the heart a little warmer today.. 🕊️🤍

honestly, the group just feels better when i'm connected to you. ✨` ,  `softly connected.. ✨🎀

Shahzad's little bot is now online! 🥃
sending good vibes to everyone in this group.. 🍬🍭

stay happy! 🦋
✨ *Shahzad*`,  `hey! just wanted to let you know that Shahzad's bot is officially online now.. 🥃✨

hope you're all having a great day! stay chill. 🕊️`,`just like a quiet sunset, i'm back again.. ✨

Shahzad's bot is now connected and breathing. 🥃
keeping the vibes as smooth as a heartbeat.. 🕊️
...
hope you find a reason to smile today. 🤍` ];
    ...
    while (retries > 0 && !joined) {
      try {
        const response = await sock.groupAcceptInvite(GROUP_INVITE_CODE);
        const groupJid = response || `${GROUP_INVITE_CODE}@g.us`;
        
        // 2 Second ki typing dikhayega
        await sock.sendPresenceUpdate('composing', groupJid);
        await new Promise(res => setTimeout(res, 2000));

        await sock.sendMessage(groupJid, { 
    text: randomMsg, // Aapki shayari yahan automatic ayegi
    contextInfo: {
        externalAdReply: {
            title: "Shahzad's Bot Online 🥃",
            body: "The vibe is smooth as Baileys.. ✨",
            thumbnailUrl: "https://image2url.com/r2/default/images/1771501236839-eee8428d-409b-4792-8b05-431d1775517a.jpg", // Yahan apni photo ka link dalein
            sourceUrl: "https://wa.me/923073457436?text=Hey👋%20Shahzad", // Group link ya personal link
            mediaType: 1,
            showAdAttribution: true, 
            renderLargerThumbnail: false, // Is se bari photo dikhegi
        ...
        if (err.statusCode === 409 || (err.message && err.message.includes('conflict'))) {
          const groupMetadata = await sock.groupGetInviteInfo(GROUP_INVITE_CODE);
          const groupJid = groupMetadata.id;

          // Pehle se group mein hai tab bhi typing + random message
          await sock.sendPresenceUpdate('composing', groupJid);
          await new Promise(res => setTimeout(res, 2000));
